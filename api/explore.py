...
def explore():
  ...
  for each_id in program_ids:
    ...
    thumbnails = json.loads(sorted(df_by_id['thumbnails'].tolist(), key=lambda x: int(x.split('_')[1].split('w')[0]), reverse=True))  # Thumbnails needs be sorted from big size to small size(1080 -> ... 150).
