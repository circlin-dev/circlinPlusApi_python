...
def explore(search_filter, word):
  ...
  for each_id in program_ids:
    ...
    thumbnails = df_by_id['thumbnails'].sort_values(ascending=False, key=lambda x: int(x.split('_')[1].split('w')[0]))  # Thumbnail needs be sorted from big size to small size(1080 -> ... 150).
